...
// Sets default values
AMoveRect::AMoveRect() : MoveRect {EMoveRect::Rect_Right}
{
...
void AMoveRect::Tick(float DeltaTime)
{
	...
	case EMoveRect::Rect_Right:
		Mesh->AddRelativeLocation(FVector(10.f, 0.0f, 0.0f));
		if (Mesh->GetRelativeLocation().X >= 500)
		{
			MoveRect = EMoveRect::Rect_Down;
		};
		break;

	case EMoveRect::Rect_Down:
		Mesh->AddRelativeLocation(FVector(0.0f, 0.0f, -10.f));
		if (Mesh->GetRelativeLocation().Z <= -500)
		{
			MoveRect = EMoveRect::Rect_Left;
		};
		break;
